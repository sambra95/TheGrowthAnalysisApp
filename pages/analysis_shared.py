...
def update_growth_stats_from_lasso(plates: dict, pid: str, well: str, chart_key: str):
    """
    ...
    xs, ys = _get_selected_points(event)
    ...
    plates.setdefault(pid, {}).setdefault("growth_stats", {}).setdefault(well, {})
    gs = plates[pid]["growth_stats"][well]
...
def _phase_controls(plate: dict, well: str, *, key: str):
    """Range slider (lag_end, exp_end) + 'No Growth' button. Writes into p['growth_stats'][well]."""
    processed_data_for_well = (plate.get("processed_data") or {}).get(well)
    if processed_data_for_well is None or processed_data_for_well.empty:
        st.warning(f"No data for {well}")
        return np.nan, np.nan, True

    t = processed_data_for_well["Time"]
...
# ---------------- Replicates (same as before) ----------------
@st.fragment
def replicates_view(plates: dict):
    ...
    with t_zoom:
        names = _sample_names(plates)
        sample = st.selectbox("Sample", names, key="rep_zoom_sample")
...
# ---------------- Window fits split into two views ----------------
...
@st.fragment
def window_plate_view(plates: dict, *, line_hours: float = 4.0):
...
@st.fragment
def window_well_view(plates: dict, *, line_hours: float = 4.0):
    a, b = st.columns(2)
    plate_id = a.selectbox("Plate", sorted(plates), key="winfit_plate")
    well = b.selectbox("Well", ALL_WELLS, key="winfit_well")

    plate_dictionary = plates[plate_id]
    key = f"{plate_id}_{well}"

    lag_end, exp_end, no_growth = _phase_controls(plate_dictionary, well, key=key)
    if no_growth:
        return

    sg_w, sg_p, _win_p = _sg_params_for_plate(plates, plate_id)

    processed_data = plate_dictionary.get("processed_data") or {}
    gs = (plate_dictionary.get("growth_stats") or {}).get(well) or {}

    # derivative figs (unchanged)
    fig_d1 = plot_window_single_d1(
        plate_dictionary, well, sg_window=sg_w, sg_poly=sg_p, frac_peak=0.20
    )
    fig_d2 = plot_window_single_d2(plate_dictionary, well, sg_window=sg_w, sg_poly=sg_p)

    chart_key = f"lasso_fit_{plate_id}_{well}"

    bg = _cached_window_single(processed_data, well)
    fig_main = go.Figure(bg)  # IMPORTANT: copy!
    fig_main = _vlines(
        fig_main,
        processed_data,
        well,
...
@st.fragment
def growth_stats_view(plates: dict):
    rows = []
    for pid, p in plates.items():
        name_map = p.get("name") or {}
        by_name = {}
        for well, nm in name_map.items():
            nm = (nm or "").strip()
        ...
        for nm, wells in by_name.items():
            wells = sorted(wells)
            rows.append((f"{pid}||{nm}", pid, nm, ", ".join(wells)))
    ...
    def _selected_ids_in_order():
        ...
        sel_ids.extend(
            sid
            for sid in ids
            if st.session_state.get(f"{sel_key}:{sid}", False) and sid not in sel_ids
        )
        return sel_ids
...
def ui_window_fits_well_editor(plates: dict, *, line_hours: float = 4.0):
    ...
    bg = _cached_window_single(processed, well)
    fig_main = go.Figure(bg)  # copy
    fig_main = _vlines(
        fig_main,
        processed,
        well,
